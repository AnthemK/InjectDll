...
void GetNewInformationBegin(int typ, int num)            //找到新的显示字符串的起点。typ==0相对模式(nunm<0向上，num>0向下)，typ==1绝对模式  单位均为行（即回车）
{
    WCHAR* NowPls, * Lastt;
    if (typ == 0 && num < 0)
    {
...
LRESULT CALLBACK FunctionDemoProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)   //调用DispatchMessage函数之后由系统来调用
{
    ...
    case WM_COMMAND:
    {
        ...
        case DEMO_BUTTON:
            ...
            SetWindowTextW(Demoedit, StrFilename); bSuccess = 0;
            ...
            if(StrFilename==NULL|| StrFilename[0]==0)  MessageBox(NULL, TEXT("并没有选择一个文件"), NULL, MB_ICONERROR);
            else bSuccess = CreateProcessW(L"..\\InjectDll\\Debug\\TestConsole.exe", StrFilename, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &startupInfo, &processInformation);//相对地址
